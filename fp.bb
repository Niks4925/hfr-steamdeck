...
Pour le choix du SSD, il vous faudra un M.2 2230.
...
Ce menu permet de configurer des réglages du Steam Deck rapidement.

[i]Performance[/i]
Les paramètres les plus importants, pour économiser la batterie et stabiliser le framerate.
Ils sont facilement accessibles dans le menu d'accès rapide et peuvent être sauvegardés pour chaque jeu individuellement via le profil situé en haut de la page.
...
[*]Contrôle manuel de l'horloge du GPU
Cette option vous permet de régler manuellement la fréquence de l'horloge du GPU.
...
[i]Proche[/i]: Également connue sous le nom de plus proche voisin, cette méthode de mise à l'échelle est la plus simple des 4 méthodes et remplace les pixels par ce qui est le plus proche pour la sortie.
Cela signifie que plusieurs pixels de la même couleur seront présents les uns à côté des autres pour essayer de créer une image de plus haute résolution.
Cette méthode permet de préserver certains détails précis, mais introduit des irrégularités dans les images lisses.
[i]Entier[/i]: Cette méthode de mise à l'échelle transforme chaque pixel d'un jeu en un groupe carré ou rectangulaire de pixels de même couleur.
Elle est très utile pour les jeux en pixel-style, mais les jeux en 3D ont tendance à avoir une netteté réduite.
[i]FSR[/i]: méthode d'upscaling open-source d'AMD.
Basée sur l'algorithme de Lanczos, cette itération de FSR (1.0) utilise une image de basse résolution anti-aliasée et utilise l'upscaling spatial pour donner l'impression que le jeu fonctionne à une résolution plus élevée.
C'est la méthode préférée pour mettre à l'échelle les jeux 3D qui, sur Steam Deck, sont ceux qui ont le plus besoin d'être configurés pour mieux fonctionner.

En combinant ces paramètres, il est généralement possible d'améliorer le fonctionnement des jeux, d'optimiser les performances et d'économiser la batterie.
Ces fonctionnalités sont intégrées à SteamOS et ne sont pas aussi facilement accessibles/disponibles dans d'autres systèmes.
